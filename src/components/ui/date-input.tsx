      ...
      <div
        className={cn(
          ...
          focused
            ...
            : "border-navy-200 hover:border-navy-300 hover:shadow-md dark:border-white/[0.1] dark:bg-white/[0.05] dark:hover:border-white/[0.15]",
          props.disabled && "cursor-not-allowed opacity-50",
        )}
      >
        {icon && (
          <Calendar className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-navy-400 dark:text-white/40" />
        )}

        {type === "date" && !hasValue && !focused && icon && (
          <span className="pointer-events-none absolute left-9 top-1/2 -translate-y-1/2 text-sm text-navy-300 dark:text-white/30">
            dd/mm/aaaa
          </span>
        )}
        ...
        <input
          ...
          className={cn(
            "flex h-full w-full rounded-xl bg-transparent py-2 text-sm text-navy-900 outline-none dark:text-white",
